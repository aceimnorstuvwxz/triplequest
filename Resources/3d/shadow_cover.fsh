...
//    vec4 shadow_color = texture2D(u_texture_shadow, v_texcoord);
     */
    vec4 col = texture2D(u_texture_shadow, gl_PointCoord);
    gl_FragColor = vec4(col.rgb, col.a * (v_start_radio + (v_end_radio-v_start_radio)*smoothstep(v_start_time, v_end_time, u_shadow_time)));

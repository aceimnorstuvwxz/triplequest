...
#define NUM_PER_EDGE_W_MIN 5
#define NUM_PER_EDGE_W_MAX 10

#define NUM_PER_EDGE_H_MIN 5
#define NUM_PER_EDGE_H_MAX 10
...
DynamicGlass* DynamicGlass::create(float width, float height, int nWidth, int nHeight, const cocos2d::Color4F& color)
{
    ...
    if (p && p->init(width, height, nWidth, nHeight, color)) {
        p->autorelease();
        return p;
    }
    CC_SAFE_DELETE(p);
...
bool DynamicGlass::init(float width, float height, int nWidth, int nHeight, const cocos2d::Color4F& color)
{
    assert(Node::init());
